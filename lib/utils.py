...
class visualize_visdom:
    def __init__(self, cfg):
        import visdom
        self.cfg = cfg
        self.n_labels = len(self.cfg['model']['labels'])
    ...
    def plot(self, epoch, train_loss, val_loss, train_acc, val_acc, confusion_matrix, auc_outs):
        ...
        # AUC curve:
        try:
            name = ['Class ' + str(i) + ' ' for i in range(self.n_labels)] + ['Micro ', 'Macro ']

            from itertools import cycle
            colors = cycle(['aqua', 'darkorange', 'cornflowerblue', 'navy', 'deeppink'])
            plt.figure()
            for i, color in zip(range(len(auc_outs[0])), colors):
                plt.plot(auc_outs[0][i], auc_outs[1][i], color=color, lw=2,
                         label=name[i] + 'ROC curve (area = %0.2f)' % auc_outs[2][i])
