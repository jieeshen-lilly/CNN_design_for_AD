...
from datasets.pet_atlas import PET_ATLAS

from lib.Loss import get_loss_criterion
from lib.utils import DataParallel_withLoss, get_auc_data, accuracy, AverageMeter, balanced_accuracy_score, \
    clip_gradients, visualize_visdom
...
cfg['file_name'] = cfg['file_name'] + '_train_perc_' + str(arguments.percentage_usage * 100) + '_expansion_' + str(
    arguments.expansion) + '.pth.tar'
cfg['exp_name'] = cfg['exp_name'] + '_train_perc_' + str(arguments.percentage_usage * 100) + '_expansion_' + str(
    arguments.expansion)
...
def main():
    ...
    train_dataset = PET_ATLAS(dir_to_scans, dir_to_tsv, "atlas_Path", "TauVisual", mode='Train',
                              labels=cfg['model']['labels'], percentage_usage=cfg['data']['percentage_usage'])
    val_dataset = PET_ATLAS(dir_to_scans, dir_to_tsv, "atlas_Path", "TauVisual", mode='Train',
...
def train(cfg, train_loader, main_model, scheduler,
    ...
    for i, (input, target, index, mmse, segment, age) in enumerate(train_loader):
        # print(i, (target, index, mmse, segment, age))
...
def validate(cfg, val_loader, main_model, criterion, epoch):
    ...
    confusion_matrix = torch.zeros(len(cfg['model']['labels']), len(cfg['model']['labels']))
    logit_all = []
    target_all = []
    ...
    target_all = torch.cat(target_all).numpy()
    acc_all = balanced_accuracy_score(target_all, np.argmax(logit_all, 1))
    plotting_fpr, plotting_tpr, roc_auc = get_auc_data(logit_all, target_all, len(cfg['model']['labels']))

    return main_losses.avg, acc_all * 100, confusion_matrix, [plotting_fpr, plotting_tpr, roc_auc]


def save_checkpoint(state, is_best, lowest_loss, is_best_micro_auc, is_best_macro_auc, is_best_auc,
