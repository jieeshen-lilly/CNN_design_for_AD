...
def prepare_model(arch='ours', in_channel=1,
    ...
    if arch == 'ours':
        image_embeding_model = NetWork(in_channel=in_channel, feat_dim=feat_dim, expansion=expansion,
                                       type_name=type_name, norm_type=norm_type)
    else:
...
def build_model(config, input_dim=3):
    ...
    if config['training_parameters']['pretrain'] is not None:
        ...
        pretrained_dict = \
            torch.load(best_model_dir + config['training_parameters']['pretrain'] + '_model_low_loss.pth.tar',
